...
from deepcore.datasets.retrieval import load_retrieval_datasets, RetrievalPaths, default_collate
from deepcore.methods.forgetting_retrieval import ForgettingRetrieval
from deepcore.methods.herding_retrieval import HerdingRetrieval
from deepcore.methods.kcentergreedy_retrieval import KCenterGreedyRetrieval
from deepcore.methods.random_retrieval import RandomRetrieval
from deepcore.nets.clip_retrieval import (
	CLIPRetrievalConfig,
	build_clip_model,
	clip_directional_accuracy,
...
def main(args):
	...
	if args.syn_init == 'kmeans':
	...
	elif args.syn_init == 'coreset':
		# args.selection = 'kcenter'
		# args.selection = 'herding'
		args.selection = 'forgetting'
		print(f'[yellow]Using coreset selection method: {args.selection}[/yellow]')

		def load_bert_embeddings(path: str) -> torch.Tensor:
			data = np.load(path)
			if "bert_test_embed" not in data:
				raise KeyError("Expected 'bert_test_embed' in NPZ file")
			return torch.from_numpy(data["bert_test_embed"]).float()
		...
		args.warmup_lr = 0.1
		args.warmup_weight_decay = 0
		args.selection_epochs = 5
		args.selection_lr = 0.1
		args.selection_weight_decay = 0.0005
		args.finetune_epochs = 100
		args.finetune_lr = 0.1
		args.finetune_weight_decay = 0.0005
		args.pair_counts = [100]
	
		paths = RetrievalPaths(dataset=args.dataset, image_root=args.image_root, ann_root=args.ann_root)
		train_dataset_coreset, eval_dataset_coreset = load_retrieval_datasets(
			paths,
			prompt=args.prompt,
		...
		bert_test_embed = load_bert_embeddings(f'./text_embeds/{args.dataset}_bert_test_text_embed.npz')

		eval_loader = DataLoader(
		eval_dataset_coreset,
		...
		def model_builder() -> torch.nn.Module:
			return build_clip_model(config)

		method_kwargs = dict(
			dataset=train_dataset_coreset,
			...
			eval_dataset=eval_dataset_coreset,
			bert_test_embed=bert_test_embed,
		)

		if args.selection == "herding":
			selector = HerdingRetrieval(**method_kwargs)
		elif args.selection == "kcenter":
			selector = KCenterGreedyRetrieval(**method_kwargs)
		elif args.selection == "random":
			selector = RandomRetrieval(**{**method_kwargs, "warmup_epochs": 0})
		else:
			selector = ForgettingRetrieval(
			**{**method_kwargs, "warmup_epochs": 0},
			train_epochs=args.selection_epochs,
			learning_rate=args.selection_lr,
			weight_decay=args.selection_weight_decay,
		)

		selection = selector.select()
		
		indices = selection['order']
		image_syn = torch.stack([train_dataset_coreset[i][0] for i in indices]).to(args.device)
		text_syn = train_caption_embed[indices].to(args.device).float()


	del eval_loader, eval_dataset_coreset, train_dataset_coreset, method_kwargs, selector
...
if __name__ == "__main__":
	...
	parser.add_argument('--syn_init', type=str, default='kmeans', choices=['random', 'kmeans','coreset'])
	...
	# -----------------------------
	parser.add_argument('--buffer_path', type=str, default='./buffer/flickr8k/nfnet_bert/InfoNCE')
