...
def load_from_timm(model_name, pretrained):
	if model_name == 'nfnet':
	   model = timm.create_model('nfnet_l0', pretrained=pretrained, num_classes=0, global_pool="avg")
		...
		model = ViTModel.from_pretrained("vit_tiny_patch16_224")
...
class CLIPModel_full(nn.Module):
	def __init__(
		...
		if train_logit_scale:
			...
			self.logit_scale = torch.ones([]) * np.log(1.0 / temperature)       # here. VL_Distill uses np.exp(np.log(1 / 0.07)). Same
	...
	def get_acc(self, image_logits, ground_truth):
		acc_i = (torch.argmax(image_logits, 1) == ground_truth).sum().item()    # argmax row: image to text
		acc_t = (torch.argmax(image_logits, 0) == ground_truth).sum().item()    # argmax col: text to image
		acc = (acc_i + acc_t) / 2
		return acc

	def get_loss(self, image_logits, ground_truth, similarity=None):
	...
	'''
	image: (B, C, H, W)
	caption: self.distill=False -> list of strings len B
	caption: self.distill=True -> (B, T) tensor of tokenized text
	'''
	def forward(self, image, caption, epoch, similarity=None):  
		...
		acc = self.get_acc(image_logits, ground_truth)
		loss = self.get_loss(image_logits, ground_truth, similarity)

		return loss, acc
	
import math 

def _safe_kaiming_uniform_(w, a=math.sqrt(5)):
	if w is None: 
		return
	if w.ndim >= 2:
		nn.init.kaiming_uniform_(w, a=a, nonlinearity='relu')
	else:
		nn.init.zeros_(w)

def _safe_xavier_uniform_(w):
	if w is None:
		return
	if w.ndim >= 2:
...
@torch.no_grad()
def _reset_module_parameters(m: nn.Module):
	"""
	모듈 단위 초기화. reset_parameters()가 있으면 먼저 호출하고,
	유형별로 안정적인 기본 초기화를 덮어씌움.
	"""
	# 0) 원래 제공 초기화 먼저
	...
	elif isinstance(m, (nn.LayerNorm, nn.GroupNorm, nn.InstanceNorm1d, nn.InstanceNorm2d, nn.InstanceNorm3d)):
		_one_(getattr(m, 'weight', None))
		_zero_(getattr(m, 'bias', None))
	...
	# 5) Embedding
		...
		# CLIP류는 작은 표준편차 정규분포가 무난
		if getattr(m, 'weight', None) is not None:
			nn.init.normal_(m.weight, mean=0.0, std=0.02)

	# 6) MultiheadAttention (혹시 TextEncoder 내부 사용 시)
	elif isinstance(m, nn.MultiheadAttention):
		if hasattr(m, 'in_proj_weight') and m.in_proj_weight is not None:
			_safe_xavier_uniform_(m.in_proj_weight)
		if hasattr(m, 'in_proj_bias') and m.in_proj_bias is not None:
			_zero_(m.in_proj_bias)
		if hasattr(m, 'out_proj'):
			_safe_xavier_uniform_(getattr(m.out_proj, 'weight', None))
			_zero_(getattr(m.out_proj, 'bias', None))

	# 7) 기타 파라미터 텐서(커스텀 모듈 대비)
	else:
		# 이름을 모르면 일반 규칙: 2D 이상은 kaiming, 1D는 zeros
...
def reset_clip_model(model: nn.Module, temperature: float = 0.07):
	"""
	CLIPModel_full 전체 초기화 + logit_scale 재설정.
	"""
	model.apply(_reset_module_parameters)

	# logit_scale 재설정(파라미터/텐서 모두 대응)
	val = torch.ones([]) * math.log(1.0 / float(temperature))
	if isinstance(getattr(model, 'logit_scale', None), nn.Parameter):
		with torch.no_grad():
			model.logit_scale.copy_(val.to(model.logit_scale.device, dtype=model.logit_scale.dtype))
	elif isinstance(getattr(model, 'logit_scale', None), torch.Tensor):
		# 텐서면 새로 할당 (장치/dtype 유지)
		dev = model.logit_scale.device
		dt = model.logit_scale.dtype
		model.logit_scale = val.to(dev, dtype=dt)