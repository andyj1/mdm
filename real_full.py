...
def main(args):
	...
	save_dir1 = os.path.join(args.buffer_path1, args.dataset)
	save_dir2 = os.path.join(args.buffer_path2, args.dataset)
	if args.dataset in ["CIFAR10", "CIFAR100"] and not args.zca:
		save_dir1 += "_NO_ZCA"
		save_dir2 += "_NO_ZCA"
	save_dir1 = os.path.join(save_dir1, args.image_encoder+"_"+args.text_encoder, args.loss_type+no_aug_suffix)
	save_dir2 = os.path.join(save_dir2, args.image_encoder+"_"+args.text_encoder, args.loss_type+no_aug_suffix)
	...
	_ = load_or_process_file('test_text', textprocess_test, args, testloader)
	_ = load_or_process_file('train_text', textprocess_train, args, train_sentences)

	data = np.load(f'{args.text_embed_dir}/{args.dataset}_{args.text_encoder}_test_text_embed.npz')
	...
	for it in range(0, args.num_experts):
		...
		teacher_net = CLIPModel_full(args, eval_stage=False).to(args.device)
		...
		img_timestamps1.append([p.detach().cpu() for p in img_teacher_net.parameters()])
		txt_timestamps1.append([p.detach().cpu() for p in txt_teacher_net.parameters()])

		img_timestamps2.append(copy.deepcopy(img_teacher_net.state_dict()))
		txt_timestamps2.append(copy.deepcopy(txt_teacher_net.state_dict()))
		...
		for e in trange(args.train_epochs):
			...
			if (e+1) % args.eval_freq == 0:
				...
				print("Itr: {} Epoch={} Train Acc={} | Img R@1={} R@5={} R@10={} R@Mean={} | Txt R@1={} R@5={} R@10={} R@Mean={} | R@Mean={}".format(
					it, e, train_acc,
		...
		if not args.skip_save:
			img_trajectories1.append(img_timestamps1)
			txt_trajectories1.append(txt_timestamps1)
			n = 0
			# while os.path.exists(os.path.join(save_dir1, "img_replay_buffer_{}.pt".format(n))):
			while any(glob.glob(os.path.join(save_dir2, f"img_replay_buffer_{n}*"))):
				n += 1
			
			print("Saving {}".format(os.path.join(save_dir2, "img_replay_buffer_{}.pth".format(n))))
			for i in range(len(img_timestamps2)):
				torch.save(img_timestamps2[i], os.path.join(save_dir2, "img_replay_buffer_{}_{}.pth".format(n,i)))
			torch.save(img_trajectories1, os.path.join(save_dir1, "img_replay_buffer_{}.pt".format(n)))

			print("Saving {}".format(os.path.join(save_dir2, "txt_replay_buffer_{}.pth".format(n))))
			for i in range(len(txt_timestamps2)):
				torch.save(txt_timestamps2[i], os.path.join(save_dir2, "txt_replay_buffer_{}_{}.pth".format(n,i)))
			torch.save(txt_trajectories1, os.path.join(save_dir1, "txt_replay_buffer_{}.pt".format(n)))
...
def make_buffer_parser():
	...
	parser.add_argument('--dataset', type=str, default='flickr30k', choices=['flickr8k', 'flickr', 'coco'], help='dataset')
	...
	parser.add_argument('--buffer_path1', type=str, default='./buffers_seed', help='buffer path')
	parser.add_argument('--buffer_path2', type=str, default='./buffers_my_seed', help='buffer path')
	...
	parser.add_argument('--eval_freq', type=int, default=1, help='eval_freq')
	...
	parser.add_argument('--text_embed_dir', type=str, default='./text_embeds')
	return parser
...
if __name__ == '__main__':
	...
	if args.dataset == 'flickr8k':
		args.image_root = '/mnt/sdc/datasets/flickr8k'
